 ...
 * @author bsimpkins
 */
public class SideScrollerManager implements Paintable
{
    private SideScrollerForeground foreground;
    ...
    public SideScrollerManager(SideScrollerForeground foreground, float parallax, Painter painter)
    {
     ...
     */
    public void addLayer(String file, int yLoc, boolean tile)
    {
        BackgroundLayer layer = new BackgroundLayer(file,painter);
        layer.setYScroll(-yLoc);
        layer.setWrap(true, tile);
    ...
    public static void main(String[] args)
    {
        ...
        SideScrollerForeground fore = new SideScrollerForeground("/files/testFile.txt",paths,painter);

        SideScrollerManager scroller = new SideScrollerManager(fore,.3f,painter)
        {
            @Override
            public void update(float interpolation)
            {
        ...
        fore.setScroll(panel.getSize().height - fore.getSize().height);
        scroller.addLayer("images/trees.gif",20,false);
        scroller.addLayer("images/houses.gif", 40,false);
        scroller.addLayer("images/mountains.gif",0,false);
     ...
     */
    public SideScrollerForeground getForeground() {
        return foreground;
    }
