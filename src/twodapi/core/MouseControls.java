...
public class MouseControls implements Runnable, Paintable
{
     ...
     */
    public MouseControls(Painter painter)
    {
    ...
    private void addToQueue(int updateType, Point mouseLocation, int button, boolean pressed, boolean entered, int index)
    {
        ...
        toAdd.setMouseLocation(getScaledPosition(mouseLocation));
    ...
    private void addToTraceQueue(Point point)
    {
        ...
        traceQueue.get(traceIndex).setLocation(getScaledPosition(point));
    }
    ...
    private boolean isOnscreen(Mouseable mouseable)
    {
        ...
        if ((location.x - size.width/2 + size.width < 0) ||
                (location.x-size.width/2 > painter.getSize().width) ||
                (location.y-size.height/2 + size.height < 0) ||
                (location.y-size.height/2 > painter.getSize().height))
        {
            return false;
        }
    ...
    public Point getScaledPosition(Point pointIn)
    {
        double xScaled = pointIn.getX()/painter.getDisplayScaleX();
        double yScaled = pointIn.getY()/painter.getDisplayScaleY();

        scaledPos.setLocation(xScaled, yScaled);
    ...
    class MyMouseListener implements MouseListener, MouseMotionListener
    {
        ...
        public void mouseDragged(MouseEvent e) 
        {
            ...
            for (int i = 0; i < mouseables.size(); i++)
            {
                ...
                Point scaledPosition = getScaledPosition(e.getPoint());
                
                boolean entered = boundsChecker.contains(scaledPosition);
    ...
    {
        Frame2D frame = new Frame2D(Frame2D.REG_PAINTER,800,600,true);

        GamePainter painter = (GamePainter)frame.getPainter();

        final MouseControls controls = new MouseControls(painter);
